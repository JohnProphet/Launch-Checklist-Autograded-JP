...
function formSubmission(document, list, pilotName, copilotName, fuelLevel, cargoMass) {
  ...
  let cargoStatus = document.getElementById("cargoStatus") // is cargo below required threshold?
      ...
      if (fuelLevel >= 10000) { // if fuel level is 10,000 or more, list stays hidden
          launchStatus.innerHTML = "Shuttle is Ready for Launch";
          launchStatus.style.color = "green";
          fuelStatus.innerHTML = "Fuel level high enough for launch";
        } else { // if fuelLevel is under 10,000, list becomes visible, "not ready" messages appear
      ...
      if (cargoMass < 10000) { //check if cargo mass is <10,000
        cargoStatus.innerHTML = "Cargo mass low enough for launch";
      } else { //if cargoMass >= 10000, show list, make red, etc.
        list.style.visibility = "visible";
        launchStatus.innerHTML = "Shuttle Not Ready for Launch";
        launchStatus.style.color = "red";
...
function pickPlanet(listedPlanets) { //pick a random planet from planets list in JSON
